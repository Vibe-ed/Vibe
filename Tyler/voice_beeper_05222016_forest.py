...
[Fs_2, Mixed] = scipy.io.wavfile.read(address + 'Single_mic_shortened.wav')
...
[Mixed_F, Mixed_t, Mixed_f_2] = spectrogram(Mixed,Fs_2, window = get_window('hann', Fs*pointsFactor), nperseg = Fs*pointsFactor, noverlap = Fs*overlapFactor) 
...
Tyler_y = np.ones(np.shape(Tyler_filtered)[1])
Henry_y = np.ones(np.shape(Henry_filtered)[1])*2
Yael_y = np.ones(np.shape(Yael_filtered)[1])*3
...
#logistic_full = AdaBoostClassifier(n_estimators = 50, random_state = 131)
logistic_full = RandomForestClassifier(n_estimators = 200, max_features = 0.25)
#logistic_full = LogisticRegression(penalty = 'l1', solver = 'liblinear', C = 0.5)
logistic_full.fit(x_shuffled, y_shuffled)
# Training Error
Tyler_predict = logistic_full.predict(Tyler_filtered.T)
Henry_predict = logistic_full.predict(Henry_filtered.T)
Yael_predict = logistic_full.predict(Yael_filtered.T)
None_predict = logistic_full.predict(None_f_2.T)
Tyler_score = logistic_full.score(Tyler_filtered.T, Tyler_y)
Henry_score = logistic_full.score(Henry_filtered.T, Henry_y)
Yael_score = logistic_full.score(Yael_filtered.T, Yael_y)
None_score = logistic_full.score(None_f_2.T, None_y)
plt.figure()
plt.title('Yael Training Prediction' + '\n' + str(Yael_score))
plt.scatter(Yael_t_filtered, Yael_predict)
plt.ylim(-1,4)
plt.figure()
plt.title('Henry Training Prediction' + '\n' + str(Henry_score))
plt.scatter(Henry_t_filtered, Henry_predict)
plt.ylim(-1,4)
plt.figure()
plt.title('Tyler Training Prediction' + '\n' + str(Tyler_score))
plt.scatter(Tyler_t_filtered, Tyler_predict)
plt.ylim(-1,4)
plt.figure()
